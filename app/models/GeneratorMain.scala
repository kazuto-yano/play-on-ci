...
object GeneratorMain extends App 
{
	val slickDriver="slick.driver.MySQLDriver"
  	...
  	val url="jdbc:mysql://localhost/ci"
  	val outputDir="./app"
  	val user=Some("root")
  	val password=Some("passwd")
  	
  	/*  	
	val pkg="srcgen.out02"
  	val tableNames:Option[Seq[String]]=Some(Seq("addresses","prefs"))
	val topTraitName="PrefAddr01"
	val scalaFileName="PrefAddr01.scala"
	*/
	val pkg="models"
  	//val tableNames:Option[Seq[String]]=Some(Seq("session_store","session_store2"))
  	val tableNames:Option[Seq[String]]=Some(Seq("hello"))
	val topTraitName="CIModel"
	val scalaFileName="CIModel.scala"
	...
	val db = { slick.driver.MySQLDriver.api.Database.forURL(url, driver = jdbcDriver, user = user.get, password = password.get) }
	//データベースよりテーブル情報を一式取得
	...
	generator.writeToFile(slickDriver, outputDir, pkg, topTraitName, scalaFileName)
